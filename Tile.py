...
class Tile:
	...
	def polygon(self):
		...
		if not self.__polygon:        
			...
			for coord in tile_coords:
				t_ra = coord.ra.degree
				if t_ra > 180:
					t_ra = coord.ra.degree - 360.

				tile_vertices.append([t_ra, coord.dec.degree])
	...
	def enclosed_pixel_indices(self):
		...
		if len(self.__enclosed_pixel_indices) == 0:
			internal_pix = hp.query_polygon(self.nside, self.corner_xyz, inclusive=True)
